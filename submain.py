...
class APP:
    def __init__(self):
        ...
        self.time_span=0.01 #名字显示间隔
        self.root.title('高一九班点题器')
        width = 680
        height = 680
        ...
        self.create_widget_for_topic()
        self.set_widget_for_topic()
        self.place_widget_for_topic()
        self.root.mainloop()

    def create_widget_for_topic(self):
        self.label_show_topics_var=StringVar()
        self.label_show_topics=ttk.Label(self.root,textvariable=self.label_show_topics_var,font=('MSYH', 30,"bold"),foreground = '#1E90FF')
        ...
        self.label_show_topics_num=ttk.Label(self.root,font=('Arial', 20),foreground = '#FF7F50')
    ...
    def set_widget_for_topic(self):
        default_topics="会是哪题？"
        self.label_show_topics_var.set(default_topics)
        self.label_show_topics_adjust(default_topics)
        ...
        init_names=self.load_names_txt("./topics.txt")
        self.root.protocol('WM_DELETE_WINDOW',self.quit_window)
        self.root.bind('<Escape>',self.quit_window)
        if init_names:
            self.default_names=init_names   #1.文件存在但是无内容。2.文件不存在
            #self.label_show_topics_num.config(text=f"一共加载了{len(self.default_names)}个姓名")
        else:
            self.btn_start.config(state=DISABLED)
            self.label_show_topics_num.config(text=f"请先手动导入人名单！")

    def place_widget_for_topic(self):
        ...
        self.label_show_topics_num.place(x=300,y=260)

    def label_show_topics_adjust(self,the_name):
        self.label_show_topics.place(x=10, y=10)
    ...
    def start_point_name(self):
        ...
        if len(self.default_names)==1:
            ...
            self.label_show_topics_var.set(self.default_names[0])
            self.label_show_topics_adjust(self.default_names[0])
            return
        ...
        if mode == "sequence":
            ...
            while True:
                    if self.running_flag:
                        ...
                        self.label_show_topics_var.set(random_choice_name)
                        self.label_show_topics_adjust(random_choice_name)
    ...
    def always_ergodic(self):
        ...
        for i in self.default_names:
            if self.running_flag:
                self.label_show_topics_var.set(i)
                self.label_show_topics_adjust(i)
                time.sleep(self.time_span)
    ...
    def load_names(self):
        ...
        if filename:
            ...
            if names:
                ...
                self.label_show_topics_num.config(text=f"一共加载了43个姓名")
                #{len(self.default_names)}
                default_topics = "会是谁？"
                self.label_show_topics_var.set(default_topics)
                self.label_show_topics_adjust(default_topics)
